...
import { FormularioEscolaridadesComponent } from "./formulario/formulario-escolaridades.component";
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, SharedModule, TituloComponent, FormularioEscolaridadesComponent]
})
...
export class EscolaridadesComponent implements OnInit, OnDestroy{
    ...
    @ViewChild(FormularioEscolaridadesComponent) formulario!: FormularioEscolaridadesComponent;
    ...
    ngOnInit(): void {
        ...
        this.data$ = this.escolaridadesService.index().pipe(
            tap(() => {
    ...
    confirm(){
        ...
        this.subscription = this.escolaridadesService.destroy(this.excluir.id || 0).subscribe({
            next: (data) => {
